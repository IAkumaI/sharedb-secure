...
const {isIdsQuery} = require('./utils');
...
module.exports = function (backend, options) {
    ...
    backend.use('apply', (req, next) => {
        ...
        if (req.originalSnapshot === undefined && !opData.create && !opData.del) {
            req.originalSnapshot = JSON.parse(JSON.stringify(req.snapshot));
        }
    ...
    backend.use('query', (req, next) => {
        ...
        if (options.options.allowRegularQueries && !query['$serverQuery']) {
            return next();
        }

        let collectionSchema = options.collections[req.collection];

        if (!collectionSchema || !collectionSchema.serverQuery) {
            return next('403: Permission denied (query), collection not found: ' + req.collection);
        }

        let queryName = query['$serverQuery'];
        let queryParams = query['$params'] || {};

        if (!queryName) {
            return next('403: Permission denied (query), collection: ' + req.collection + ': empty $serverQuery');
        }

        let queryFunction = collectionSchema.serverQuery[queryName];

        if (!queryFunction) {
            return next('403: Permission denied (query), collection: ' + req.collection + ', server query not found: ' + queryName);
        }

        queryFunction(queryParams, req, (err, realQuery) => {
            if (err) return next(err);
            req.query = realQuery;
            return next();
        });
