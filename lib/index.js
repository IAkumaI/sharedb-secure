...
module.exports = function (backend, options) {
    ...
    backend.use('query', (req, next) => {
        ...
        if (queryName) { // Always process server query if provided or sharedb will fail
            let queryFunction = collectionSchema.serverQuery && collectionSchema.serverQuery[queryName];
            let queryParams = query['$params'] || {};

            if (!queryFunction) {
                return next('500: Server query not found, collection: ' + req.collection + ': ' + queryName);
            }
        ...
        } else { // Check access if no server query provided
            if (options.options.allowRegularQueries) {
                return next();
            }
            ...
            return next('403: Permission denied (query), you can not run regular queries: ' + req.collection);
        }
    });
};