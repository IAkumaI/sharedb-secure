        ...
        async.each(req.snapshots, function (snapshot, cb) {
            ...
            collectionSchema.getRole(docId, doc, session, req, (err, role) => {
                ...
                accessHandler.check(docId, doc, session, req, (err) => {
                    if (err) return cb('403: Permission denied (read, no access), collection: ' + req.collection + ', docId: ' + docId + ', role: ' + role);
    ...
    backend.use('apply', (req, next) => {
        ...
        if (opData.create) {
            ...
            collectionSchema.getRole(docId, doc, session, req, (err, role) => {
                ...
                accessHandler.check(docId, doc, session, req, (err) => {
                    if (err) return next('403: Permission denied (create, no access), collection: ' + req.collection + ', docId: ' + docId + ', role: ' + role);
            ...
            collectionSchema.getRole(docId, doc, session, req, (err, role) => {
                ...
                accessHandler.check(docId, doc, session, req, (err) => {
                    if (err) return next('403: Permission denied (delete, no access), collection: ' + req.collection + ', docId: ' + docId + ', role: ' + role);
            ...
            // Для Обновлений
            if (req.originalSnapshot === undefined) {
    ...
    backend.use('apply', (req, next) => {
        ...
        if (collectionSchema.validators && collectionSchema.validators.create) {
            collectionSchema.validators.create(docId, doc, session, req, (err) => {
                if (err) return next('403: Permission denied (create, invalid), collection: ' + req.collection + ', docId: ' + docId);
    ...
    backend.use('commit', (req, next) => {
        ...
        collectionSchema.getRole(docId, oldDoc, session, req, (err, role) => {
            ...
            accessHandler.check(docId, oldDoc, newDoc, session, req, (err) => {
                if (err) return next('403: Permission denied (update, no access), collection: ' + req.collection + ', docId: ' + docId + ', role: ' + role);
    ...
    backend.use('commit', (req, next) => {
        ...
        if (collectionSchema.validators && collectionSchema.validators.update) {
            collectionSchema.validators.update(docId, oldDoc, newDoc, session, req, (err) => {
                if (err) return next('403: Permission denied (update, invalid), collection: ' + req.collection + ', docId: ' + docId);
